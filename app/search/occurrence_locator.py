...
class OccurrenceLocator:
    ...
    def locate_exact(self, text: str, terms: list[str], *, is_phrase: bool = False) -> list[LocatedOccurrence]:
        ...
        for term in terms:
            ...
            while True:
                ...
                results.append(self._build(text, index, end, term, "exact"))
                start = end if is_phrase else index + 1
        return results

    def locate_lemmas(self, text: str, lemmas: list[str]) -> list[LocatedOccurrence]:
        ...
        for match in TOKEN_RE.finditer(text):
            ...
            if lemma in lemma_set:
                results.append(self._build(text, match.start(), match.end(), lemma, "lemma"))
        return results

    def _build(self, text: str, start: int, end: int, query_term: str, match_type: str) -> LocatedOccurrence:
